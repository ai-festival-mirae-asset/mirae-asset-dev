...
> 규격의 출처는 주최 공식 자료 두 가지 — `AI페스티벌_과제설명_금융상품Agent_참여자공유용.pdf` p.11(요청/응답 상세)과 8/13 배포 안내문 `API 호출 관련 참고.txt`(호출 방식·서버 구성) — 이며, 아래 예시 응답은 **실제 서버가 낸 값**을 그대로 붙였습니다(8/26 재배포본 기준, AI 호출 없는 가벼운 구성).
...
| 항목 | 값 |
|---|---|
| **End-point URL** | **`http://49.50.143.28/answer`** — NCP 서버 `mirae-api-01`(한국 리전, Ubuntu 24.04)의 공인 IP. 9/2 배포·외부 응답 확인(`/health` 항목 전부 정상, HCX 포함 실전 구성). 개발 중에는 `http://127.0.0.1:8000/answer` |
...
| `answer` | string | 최종 답변 | 근거로 확인된 사실만으로 작성. 확인 불가일 때는 정해진 문장 `요청하신 내용은 보유 데이터 기준으로 확인할 수 없습니다.` 로 시작하고 사유를 붙인다. 답변 끝에 데이터 기준일(국내 마스터 2026-08-22 · 해외 2026-08-23 · 구성종목 2026-08-21)을 항상 표기 |
...
| 처리 시간 초과 | 요청마다 시간 예산을 둔다 — 경과 6초 뒤에는 의미 검색을, 7초 뒤에는 AI 문장 생성을 건너뛰고 더 가벼운 방법으로 답한다(강등, `think_trace` 에 기록). 주최는 응답 시간 기준을 공개하지 않는다(8/26 공지 — 300초 초과만 미응답 처리, 늦은 응답은 점수 차등). **내부 목표 15초**(문서 권장 60초·타임아웃 300초) |
| 빈 질문 / 파라미터 누락 | §2 표 참조 — 200 + 안내 문구 |
| 규격 밖 파라미터 | 무시 |
| 5xx | 설계상 발생하지 않음(전역 예외 처리) — 만약 서버 프로세스 자체가 죽으면 systemd 가 자동 재기동(배포 가이드 `infra/deploy/README_DEPLOY.md`) |
...
| 문항당 60초 이내 권장 (PDF p.11) / 타임아웃 300초·재시도 2회 (안내문) / 응답 시간 기준 비공개 (8/26 공지) | 내부 목표 15초, 단계별 자동 강등 | 실측 시험지 8종 403문항 15초 초과 0건 · 최대 13.2초 (`evalset/reports/`) |
