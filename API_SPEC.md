...
| 처리 시간 초과 | 요청마다 시간 예산을 둔다 — 경과 6초 뒤에는 의미 검색을, 7초 뒤에는 AI 문장 생성을 건너뛰고 더 가벼운 방법으로 답한다(강등, `think_trace` 에 기록). **내부 목표 15초 = 무감점 경계**(설명회 발화 — 15초 이하 감점 없음, 초과 시 감점·방식 미공개. 8/22 팀 리더 확인). 문서 규정: 권장 60초·타임아웃 300초 |
...
| 문항당 60초 이내 권장 (PDF p.11) / 타임아웃 300초·재시도 2회 (안내문) / **15초 이하 무감점**(설명회 발화, 8/22 확인) | 내부 목표 15초, 단계별 자동 강등 | 실측 중앙값 2.84초·최대 9.24초·15초 초과 0건 (8/19 성적표) |
