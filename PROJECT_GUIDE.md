...
### 외부 데이터 3-Tier 정책 (papuagigi 브랜치에서 차용, 2026-08-05)

채점 정답지가 제공 데이터로 만들어진다는 점을 뒤집어 읽으면, 외부 데이터는 **값 대체가 아니라 제공 데이터 해석에 쓸 때만 ROI가 난다.** 위 5개 원칙을 아래 3단계로 운용한다.

| Tier | 대상 | 방침 |
|---|---|---|
| **1 — 적극 수집** | 컬럼·값 사전, 신용등급·위험등급 체계, 통화·시장 코드표, 브랜드 별칭(“타이거”=TIGER=미래에셋), 기초지수 명칭 정규화 | 제공 데이터를 **정확히 해석**하기 위한 메타데이터. 값을 바꾸지 않으므로 충돌 위험이 없다 |
| **2 — 조건부** | 공모펀드 보수, 국내 ETF 총보수·기초지수 등 결측 필드 보강 | 1차 답변은 항상 제공 데이터 기준(“확인할 수 없음”). 외부 값은 별도 테이블 + 출처 컬럼으로만 저장하고 `retrieved_context`에서 출처를 구분 표기한다. 08-06 예시 질의 확인 후 진입 여부 재판단 |
| **3 — 금지** | 시세·수익률 등 시계열 대체 데이터 | 스냅샷 기준일과 반드시 충돌하므로 규칙 위반. 평가 리스크만 증가 |

핵심은 Tier 1 사전이 결측을 **메꾸는** 자산이 아니라 결측을 **정확히 판별하고**(센티널 인식) 있는 값을 정확히 해석하기 위한 자산이라는 점이다. 해외 ETF 기초지수가 “결측 0%”로 보이지만 Lipper 센티널 2종을 인식하면 실질결측 47.9%인 사례가 이를 그대로 보여준다.
...
- `term_dictionary`: 컬럼·값·별칭·등급·코드 해석 사전. papuagigi 브랜치 `외부데이터/사전/` 9종(컬럼사전 207행 전 컬럼 커버, 값사전 423행, 신용등급·위험등급·채권분류·코드표·펀드클래스·기초지수·별칭 110행)을 적재원으로 삼는다. 각 행의 `신뢰도`·`검증상태`·`출처`를 그대로 보존하고, 답변에 사전 해석을 쓴 경우 `retrieved_context`에 사전 출처를 표기한다
- `product_axis`: 상품군을 가로지르는 공통 분류 축. schema `Sheet2_Sample`에만 있고 datarows에는 없는 `axis_*` 컬럼(채권 8종·국내 ETF 7종·공모펀드 6종)은 주최 측이 남긴 교차 질의용 축 힌트이며, 그 축을 채우는 일이 참가팀 몫이다. 채권·ETF·펀드를 같은 언어로 질의하는 유일한 경로이므로 기술제안서의 핵심 챕터로 삼는다
...
아래 22~26은 papuagigi 브랜치에서 차용하고 2026-08-05에 우리 원본으로 전수 재검증한 규칙이다.

22. 해외 ETF `cu_base_index`의 Lipper 센티널 2종을 null로 변환한다. `Index is not provided by Management Company` 1,984건 + `Index is not available on Lipper Database` 721건 = 2,705건(47.9%)이다. 공백 8건을 더하면 실질 유효 기초지수는 2,933건(51.9%)뿐이다. 센티널을 문자열로 두면 `IS NULL`이 작동하지 않아 “확인할 수 없음”으로 답해야 할 질의에 쓰레기 값으로 답하게 된다.
23. 채권 `CRD_GRD`의 끝자리 `0`은 무부호(플랫) 표기다. `AA0` 1,713건은 `AA`와 같은 등급이며 끝자리 0 표기는 전체 2,886건이다. 정규화 없이 “AA 이상” 필터를 걸면 1,713건이 통째로 누락된다. `AAA=1`~`D=20`의 `crd_grd_rank`를 만들어 서열 비교를 SQL로 처리하고, “AA 이상”은 `rank<=3`으로 고정한다. 단 `C0` 137건이 진짜 C등급인지 무등급 코드 오버로딩인지는 미확인이므로 rank 매핑 전에 확인한다.
24. 채권 `PD_EVCO_CRD_GRD`는 평가사별 등급 콤마 병기다(비공백 24,966건 중 22,605건이 콤마 포함, 예: `AAA, AAA, AAA`). 개수와 최저등급으로 분해하고, 평가사 간 불일치(split) 시 보수적으로 최저등급을 채택한다. 병기 순서와 평가사 매칭은 미확인이므로 순서에 의미를 부여하지 않는다.
25. 값이 한쪽 값만 관측되는 플래그 컬럼의 null은 결측이 아니라 반대값이다. 해외 ETF `cu_etn_yn`(Y 59건), `cu_inverse_short_yn`(Y 171건)이 해당하며 null을 명시적 `N`으로 채운 파생 컬럼을 만든다. **단 컬럼마다 관측 분포를 먼저 확인한다** — `wu_core_yn`은 관측값이 `N` 103건뿐이라 같은 규칙을 적용할 수 없고, `cu_index_tracking_yn`은 Y 2,360건에 공백 3,286건이라 결측과 N을 구분할 수 없다.
26. 국내 ETF 연금 플래그는 교차 검증으로 의미가 확정됐다. `pd_pen_risk_nm`의 위험자산 788 + 안전자산 214 = 1,002건이 `pd_pen_tr_yn='Y'` 1,002건과 정확히 일치하고 교차표가 완전한 대각선이다. 따라서 `pd_pen_risk_nm='N'` 732건은 위험구분 결측이 아니라 **연금거래 불가**를 뜻한다.
...
- 센티널 인식: 해외 ETF `cu_base_index` 센티널 2,705건이 null로 변환되고 실질 유효 기초지수가 2,933건인지 확인
- 등급 정규화: `AA0`→`AA` 1,713건 변환 후 `crd_grd_rank<=3`이 AAA·AA+·AA를 모두 포함하는지 확인
- 플래그 의미: `pd_pen_risk_nm`의 위험/안전자산 1,002건과 `pd_pen_tr_yn='Y'` 1,002건이 계속 일치하는지 확인 (불일치 시 N=연금거래불가 해석을 재검증)
- 결측 boolean: 원본이 결측인 행의 파생 boolean이 `false`가 아니라 null인지 확인 (결측을 “아니오”로 접지 않는다)
...
### 브랜치 통합 계약 4종 (papuagigi 브랜치에서 차용, 최우선)

팀이 `dev-kyung`·`papuagigi`·`jhnam` 세 브랜치로 갈라져 각자 개발 후 최적안을 통합하는 방식이므로, **비교 가능한 공통 계약을 먼저 고정**해야 한다. 계약이 없으면 통합 시점에 우열 비교 자체가 불가능하다.

1. 도구 5종의 JSON Schema (Function Calling 파라미터 정의)
2. DB 스키마 (`product_axis` 포함 — 테이블·컬럼·타입)
3. 응답 4필드 JSON (`question_id` / `retrieved_context` / `think_trace` / `answer`의 내부 형식)
4. 자체 평가셋 + 공용 회귀 러너 (브랜치 무관하게 같은 명령으로 정답률 산출)

이 4개가 같으면 브랜치별 정답률이 객관적 통합 기준이 되고, “A의 라우터 + B의 도구 + C의 가드레일” 식 부분 통합도 가능해진다.

### 작업 순서

1. PostgreSQL DDL과 `fund_master`, `fund_attribute`, `taxonomy_mapping`, `term_dictionary`, `product_axis`, quarantine 스키마를 작성한다.
