...
_COL_DISPLAY = {
    ...
    "pd_net_tamt": ("순자산총액", "krw"), "du_last_aum": ("순자산(AUM)", "krw"), "total_aum": ("순자산 합계", "krw"), "n_with_aum": ("순자산 값 보유 건수", "int"), "weight_sum": ("비중 합계", "pct"),
    "weight_a": ("첫 상품 편입 비중(%)", "pct"), "weight_b": ("둘째 상품 편입 비중(%)", "pct"),
...
_ATTR_NOTES = [
    ...
    (r"보수|수수료(?!\s*(?:유형|미징구|선취|후취))|더\s*싸|싼|저렴", "cu_charge_rt", "총보수(%)", "text"),   # 16바퀴: 수수료 얼마 · 19바퀴: 더 싸
    ...
    (r"^(?!.*(?:[136]\s*개월|한\s*달|배당\s*수익률|분배\s*수익률)).*수익률", "fd_yr1_ern_r", "1년 수익률(%)", "text"),   # 15바퀴: 배당수익률 질문에 1년 수익률이 먼저 붙던 것
    (r"^(?!.*(?:[136]\s*개월|한\s*달|배당\s*수익률|분배\s*수익률)).*수익률", "du_er_1y", "1년 수익률(%)", "text"),
    (r"ETF야|ETN이야|유형|종류", "drv_instrument_type", "상품 유형", "text"),
    # 8/27 재배포본 신설 분배·품질 필드 (구본은 전부 0/결측이라 거절하던 항목)
    (r"배당금|분배금", "pd_divd_amt_ann", "연간 추정 분배금(원)", "text"),   # 15바퀴: '분배금 얼마 줘'는 금액이 먼저
    (r"배당\s*수익률|분배\s*수익률|배당(?!금)|분배(?!금)", "pd_dvid_yield", "분배(배당)수익률(%)", "text"),
    ...
    (r"1\s*개월\s*변동성|변동성\s*1\s*개월", "du_vlty_1m", "1개월 변동성(%)", "text"),
    (r"3\s*개월\s*변동성|변동성\s*3\s*개월", "du_vlty_3m", "3개월 변동성(%)", "text"),
    (r"6\s*개월\s*변동성|변동성\s*6\s*개월", "du_vlty_6m", "6개월 변동성(%)", "text"),
    (r"^(?!.*(?:[136]\s*개월\s*변동성|변동성\s*[136]\s*개월)).*변동성", "du_vlty_1y", "1년 변동성(%)", "text"),
    ...
    (r"종가|가격|한\s*주|1\s*주|주당|얼마짜리|얼마\s*(?:에|면)\s*(?:사|살)", "du_clpr", "장내 종가(원)", "text"),   # 15바퀴: 'KODEX 200 한 주 얼마야'
    (r"거래소|어느\s*시장|무슨\s*시장|시장\s*(?:에|은|이)?\s*상장|유가증권\s*시장|코스피\s*시장|코스닥\s*시장", "pd_mkt_nm", "상장 시장", "text"),   # 15바퀴: '어느 거래소에 상장돼 있어'
...
_ONLY_FIELDS = [                                          # (질문 낱말, 후보 열, 라벨, 형식) — 앞에서부터 매칭·소비
    ...
    (r"총\s*보수|보수|수수료(?!\s*(?:유형|미징구|선취|후취))|더\s*싸|싼|저렴", ("cu_charge_rt", "total_fee_pct"), "총보수", "pct"),   # 19바퀴: '뭐가 더 싸'   # 16바퀴: 'KODEX 200 수수료 얼마'
