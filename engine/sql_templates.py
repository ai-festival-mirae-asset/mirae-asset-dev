...
TEMPLATES = {t.id: t for t in [
    ...
    _t("etp_top_aum",
       ...
       """SELECT pd_itm_no, pd_abrv_nm, pd_net_tamt, coalesce(m.resolved, cu_fund_mgmt_co) AS cu_fund_mgmt_co,
                 drv_risk_grade, pd_lstg_dt
          FROM kr_etp LEFT JOIN mgmt_resolved m USING (pd_itm_no)   -- 12바퀴: 운용사 표기는 오염 정정값
       ...
       """SELECT pd_itm_no, pd_abrv_nm, pd_lstg_dt FROM kr_etp
          WHERE drv_instrument_type = 'ETF'
            AND replace(coalesce(pd_lstg_dt,''),'-','') BETWEEN replace($date_from,'-','')
                                                            AND replace($date_to,'-','')
          ORDER BY (CASE WHEN $order = 'asc' THEN -1 ELSE 1 END)
                   * TRY_CAST(nullif(replace(coalesce(pd_lstg_dt,''),'-',''),'') AS BIGINT) DESC NULLS LAST,
                   pd_itm_no LIMIT $limit""",
       [Param("date_from", required=True), Param("date_to", required=True),
        Param("limit", required=True), Param("order", enum=("asc",))],
       source="PREF01N001", key_col="pd_itm_no"),
    ...
    _t("etp_count",
       ...
       """SELECT drv_instrument_type, drv_listing_status, count(*) AS n FROM kr_etp
          WHERE ($min_aum_gt IS NULL OR TRY_CAST(pd_net_tamt AS DOUBLE) >  $min_aum_gt)
            ...
            AND ($mgmt IS NULL OR coalesce((SELECT m.resolved FROM mgmt_resolved m WHERE m.pd_itm_no = kr_etp.pd_itm_no),
                                           cu_fund_mgmt_co) LIKE $mgmt || '%')   -- 12바퀴 운용사 범위 건수(숨김)
       ...
       [Param("min_aum_gt"), Param("min_aum_ge"), Param("max_aum_lt"), Param("max_aum_le"),
        ...
        Param("pension_only"), Param("max_er_1y"), Param("min_er_1y"), Param("index_pattern"), Param("mgmt")],
       source="PREF01N001"),
    ...
    _t("etp_metric_avg",
       ...
       """SELECT round(avg(CASE $metric WHEN 'diff' THEN TRY_CAST(du_diff_rt AS DOUBLE)
          ...
          WHERE drv_listing_status = 'active'
            AND ($type IS NULL OR drv_instrument_type = $type)
            AND ($name_pattern IS NULL OR pd_nm ILIKE $name_pattern ESCAPE '\\')   -- 9/6 6바퀴 브랜드 표기(숨김)
            AND ($mgmt IS NULL OR coalesce(m.resolved, e.cu_fund_mgmt_co) = $mgmt
    ...
    _t("bond_metric_avg",
       "국내채권 수치 항목 평균 집계 — 9/6 6바퀴('국고채 표면금리 평균 얼마야'가 목록으로 답하던 것). "
       "metric: coupon(표면금리)/after_tax(세후수익률)/dur(듀레이션). 값 0·결측 제외 단순 평균(발행액 가중 아님). "
       "규칙 라우터 전용(AI 라우터 목록에서 숨김 — LLM_HIDDEN_TEMPLATES).",
       """SELECT round(avg(CASE $metric WHEN 'coupon' THEN TRY_CAST(SRFC_IRT AS DOUBLE)
                                  WHEN 'after_tax' THEN TRY_CAST(AFTER_TAX_YIELD AS DOUBLE)
                                  ELSE TRY_CAST(DUR AS DOUBLE) END), 2) AS avg_value,
                 count(*) AS n
          FROM kr_bond
          WHERE ($currency IS NULL OR CURR_CD = $currency)
            AND ($max_rating_rank IS NULL OR TRY_CAST(drv_crd_grd_rank AS INT) <= $max_rating_rank)
            ...
            AND ($min_issue_dt IS NULL OR ISU_DT >= $min_issue_dt)
            AND ($max_issue_dt IS NULL OR ISU_DT <= $max_issue_dt)
            AND coalesce(CASE $metric WHEN 'coupon' THEN TRY_CAST(SRFC_IRT AS DOUBLE)
                                      WHEN 'after_tax' THEN TRY_CAST(AFTER_TAX_YIELD AS DOUBLE)
    ...
    _t("global_etf_metric_avg",
       "해외ETF 총보수·순자산 평균 집계 — 9/6 6바퀴('해외 ETF 총보수 평균 얼마야'가 폴백이던 것). "
       "metric: fee(총보수 %)/aum(순자산, 달러). 값 0·결측 제외 단순 평균. 자산유형·지역·운용사·ETF/ETN·인버스 조건 선택. "
       "규칙 라우터 전용(AI 라우터 목록에서 숨김 — LLM_HIDDEN_TEMPLATES).",
       """SELECT round(avg(CASE $metric WHEN 'aum' THEN TRY_CAST(du_last_aum AS DOUBLE)
                                  ELSE TRY_CAST(cu_charge_rt AS DOUBLE) END), 2) AS avg_value,
                 count(*) AS n
          FROM global_etf
          WHERE ($region_pattern IS NULL OR wu_inv_rgn ILIKE $region_pattern ESCAPE '\\')
            AND ($ast_type IS NULL OR wu_inv_ast_type = $ast_type)
    ...
    _t("global_etf_aum_sum",
       "해외 ETF 순자산(du_last_aum, 달러) 합계·건수 — 12바퀴 '해외 ETF 전체 순자산 합계'(종전 국내 합계로 오답). "
       "상품명 조건 선택 · 규칙 라우터 전용(숨김).",
       """SELECT count(*) AS n, count(TRY_CAST(du_last_aum AS DOUBLE)) AS n_with_aum,
                 sum(TRY_CAST(du_last_aum AS DOUBLE)) AS total_aum, 'USD' AS pd_trd_ccy
          FROM global_etf
          WHERE drv_instrument_type = 'ETF'
...
LLM_HIDDEN_PARAMS = {
    ...
    # 9/6 리더 6바퀴
    ("fund_top_return_1y", "region"), ("fund_top_return_1y", "name_pattern"),
    ("global_etf_filter", "mgmt_pattern"), ("global_etf_count", "mgmt_pattern"),
