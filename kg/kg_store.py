...
# 상품 클래스 IRI (ontology/finance.ttl)
PRODUCT_CLASSES = {MF + c for c in
                   ("Bond", "ETF", "ETN", "ExchangeTradedProduct", "PublicFund")}
...
class TripleStore:
    ...
    def search_products(self, query, limit=10):
        ...
        for s in self.product_subjects():
            names = self.objects(s, RDFS_LABEL) + self.objects(s, MF + "shortName")
            for n in names:
                if q in norm_name(n):
