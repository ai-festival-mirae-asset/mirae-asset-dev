...
| 데이터 프로파일링 코드 | 완료 | `데이터_프로파일링/` (재실행 가능 스크립트·컬럼별 CSV) |
| 해석 메타데이터 사전 | 완료 | `해석_메타데이터/` 9종 — 컬럼 207개 전체 해석, 별칭·등급·코드·지수 사전, 출처·검증상태 포함 |
| 전처리 파이프라인 | 1차 완료 | `scripts/preprocess.py` → `data/processed/` CSV 4종 + quarantine 1행 + 규칙 리포트 56건 |
| 전처리·수집 방법 문서 | 완료 | `데이터_전처리_방법.md`, `외부데이터_수집_방법.md`, `금융상품Agent_프로젝트_로드맵.md` |
...
- [ ] `du_lpr`의 스키마 한글명 '시가'가 저가(low)의 오기인지 확인 필요 (전체 질문: `해석_메타데이터/수집_요약.md` 5장)
...
- 변경: `해석_메타데이터/` 사전 9종(웹 리서치 428건, 적대 검증 정정 13건 반영), `scripts/preprocess.py`와 `data/processed/`(CSV 4종·quarantine·규칙 리포트 56건), `데이터_전처리_방법.md`·`외부데이터_수집_방법.md`·`금융상품Agent_프로젝트_로드맵.md`·`데이터_프로파일링/` 추가
...
- blocker: `NDY_*` 공식 정의, `CRD_GRD` 대표값 규칙, 금액 단위 미확인 — 8/6 설명회 질문 리스트로 정리(`해석_메타데이터/수집_요약.md` 5장)
