...
- `manifest/`에는 공식 과제 소개 PDF와 텍스트본만 둔다. Lance `2.manifest`는 참조 fragment가 없어 복구 불가하고 온톨로지·pgvector 경로에서 쓰이지 않아 2026-08-06에 저장소에서 제거했다.
...
- 배점은 소스코드 20점 + 기술제안서 40점 + API 호출·답변 40점이다. 기술제안서 비중이 가장 크다.
- 평가 질의는 총 30개(상 10·중 10·하 10)이고, **답변 불가 질의 5개가 별도로 있으며 답변하면 감점**이다. 답변 가능성 판정이 곧 점수다.
- 응답 시간도 평가 항목이며 실서비스 기준 15초다. 전체 제출자 평균 대비로 감점될 수 있다.
- **온톨로지 파일이 GitHub 제출물에 포함된다.** 기술제안서에도 온톨로지 설계 근거를 넣어야 한다.
- 기술제안서는 PDF와 Word 두 형식으로 내며 시스템 구성도와 주요 기능 흐름도를 포함한다.
- 최종 답변 생성은 HyperCLOVA X 고정이고 권장 모델은 `HCX-005`다. CLOVA Studio를 거치지 않고 API 키 방식으로 직접 통신한다.
- API 응답에 근거(출처) 포함이 필수이고, 확인 불가 질의는 "확인할 수 없음"을 명시해야 한다.
- 존재하지 않는 상품이나 없는 신용등급을 답하면 감점이다. 데이터에 없는 주관적 전망·추천도 금지다.
- 데이터 기준일은 2026-07-11이며 이후 생성된 ETF 등은 평가 대상이 아니다. 우리 스냅샷이 그대로 기준이다.
- 서버는 NCP에 배포하고 평가자가 직접 API를 호출한다. VPC → 서브넷 → 서버 → ACG 순서로 구성한다.
- 크레딧은 20만원이고 유효기간은 09-30이다. 기한 후 상품이 남아 있으면 비용이 청구될 수 있다.
- 평가는 UI를 제외하고 API 응답 성능만 본다. 본선 라이브 데모는 별도다.
- 주최 측은 스키마만 제공한 NL2SQL 정확도를 8.3%로 제시하며 온톨로지 활용 시 향상을 주장했다. 복합 검색(벡터+키워드+그래프+SQL)을 권장했다.

## 현재 설계 결정

- 운영 저장소는 PostgreSQL을 기본으로 한다. 다만 **온톨로지 파일이 필수 제출물**이므로 08-06 이전의 "GraphDB·온톨로지 미사용" 결정은 폐기했다. RDF/OWL 온톨로지를 별도 산출물로 작성하고 정제층과 개념·제약을 일치시킨다. 질의 실행은 계속 SQL이 주 경로다.
- 온톨로지는 값 자체가 아니라 **값의 존재·출처·기준일**을 모델링한다. 지표마다 `Observation` 노드로 값·`isObserved`·`hasSource`·`asOfDate`를 묶어 결측을 0으로 채우지 못하게 한다.
...
- [x] 08-06 설명회 반영 완료. 아래 "08-06 설명회 확정 사항" 참조
- [ ] 온톨로지 인스턴스 그래프(지식 그래프) 생성 범위 결정 필요. 스키마는 `ontology/finance_product.ttl`로 작성했고, 60,911개 상품 전체를 트리플로 내릴지 상품군별 표본으로 낼지 미정
- [ ] NCP 계정·크레딧 신청, VPC/서브넷/서버/ACG 구성, CLOVA Studio API 키 발급 필요
- [ ] HTTPS 요건 충족 시 글로벌 DNS + Certificate Manager 상품 추가 필요
...
- 반입: `manifest/과제소개_금융상품Agent.md`, `external_data/펀드별 보수비용비교_20260805.xls`(KOFIA 원본 7.7MB), `config/taxonomy_mapping_proposal.csv`, `reference/jhnam/` 12개 파일
...
### 2026-08-06 — KRX 외부 보강(Tier 2) 반입과 미해결 항목 3건 해소

- 목표: `origin/jhnam` 신규 커밋 4건에서 가져올 것 반입
- 반입: `external_data/KRX_ETF_종목정보_20260806.xlsx`(1,160행), `KRX_ETN_종목정보_20260806.xlsx`(370행), `reference/jhnam/enrich_domestic_etf_fees.py`
- 변경: `pipeline/enrich_external.py` 신규. 공식값을 절대 덮어쓰지 않고 외부값을 별도 컬럼·출처로 저장한다. 산출물은 `artifacts/data/reference/external_domestic_etp.csv.gz`
- 검증: 정제본 1,733행 중 1,492건 매칭(ETF 1,132 / ETN 360). 단위 테스트 49건 통과
- 해소 1: 국내 ETF 총보수 0의 의미. 공식·외부 값이 모두 있는 215건 중 201건(93.5%)이 상충하고, 그중 **148건은 공식값이 0인데 KRX는 양수**다. 0을 실제 무보수로 집계하지 않은 기존 판단이 외부 근거로 뒷받침된다. 커버리지 3.87% → 86.09%
- 해소 2: 국내 ETF 기초지수. 커버리지 3.35% → 89.32%. 공식값이 있던 활성 ETF 2건은 **둘 다 오기**였다. `KR70193M0005`의 실제 지수는 `KAP 27-12 특수채 총수익 지수(AAA이상)`, `KR70207Z0004`(키움 미국우주데이터센터인프라 주식형)의 실제 지수는 `iSelect 미국 우주데이터센터인프라 지수`인데 둘 다 `종합채권 1~2년`으로 기록돼 있었다. 즉 공식 기초지수의 실효 커버리지는 2건이 아니라 0건이다
- 해소 3: 활성 상품 만기일. KRX ETN 종목정보의 `최종거래일`로 활성 ETN 381건 중 **360건** 보강 가능(범위 2026-09-04 ~ 2039-10-26). 공식 데이터에 없다는 결론은 유지하되 외부 보강 경로가 생겼다
- 교차검증: jhnam이 발견한 `pd_itm_no` 접두사 규칙(`KR7`=ETF, `KRG`=ETN)을 우리 `pd_grp_no` 기반 분류와 대조한 결과 1,733행 **전부 일치**했다. 유일한 불일치는 격리한 Excel 1,155행(`pd_itm_no='KR'`)이며, 이 행이 손상됐다는 세 번째 독립 근거다
- 결정: 외부값은 공식값이 관측되지 않은 자리에만 채우고, 상충은 `expense_ratio_conflict`·`base_index_conflict` 플래그로만 남긴다. 공식값 덮어쓰기 금지를 assertion으로 고정했다
- blocker: Tier 2 사용 승인 필요. 08-06 설명회에서 외부 보강 허용 범위 확인 후 통합 뷰에 연결한다

### 2026-08-06 — 설명회 제약 반영과 온톨로지 착수

- 목표: 08-06 설명회에서 확정된 제약을 설계에 반영하고, 신규 필수 제출물인 온톨로지에 착수
- 변경: `ontology/finance_product.ttl` 신규 작성. rdflib 파싱 검증 통과(275 트리플, 클래스 29·ObjectProperty 11·DatatypeProperty 16)
- 변경: `requirements.txt`에 `rdflib==7.6.0` 추가
- 폐기: "GraphDB·온톨로지 미사용" 결정. 온톨로지가 필수 제출물이므로 유지할 수 없다
- 설계: 온톨로지는 값이 아니라 값의 맥락을 모델링한다. `Observation` 노드가 값·`isObserved`·`hasSource`·`asOfDate`를 묶어 결측을 0으로 채우는 경로를 구조적으로 막는다
- 설계: `UnsupportedField`와 `NoMatchingProduct`를 별도 개별자로 두어 "데이터에 없음"과 "조건에 맞는 상품 없음"을 어휘 수준에서 분리했다. 답변 불가 질의 5개 감점 항목에 직접 대응한다
- 설계: `ETF`와 `ETN`을 `owl:AllDisjointClasses`로 선언했다. 파일명이 ETF 마스터인데 30.7%가 ETN인 문제를 스키마에서 차단한다
- 설계: 위험등급 1~6, 신용등급 서열 1~20을 `owl:withRestrictions`로 제약해 원본의 위험등급 0과 미매핑 등급이 인스턴스화되지 않게 했다
- 근거 축적: 지금까지 만든 `field_policy`·`answerability`·`quality_status`·`listing_status`가 그대로 온톨로지 어휘가 됐다. 기술제안서 40점의 "온톨로지 설계 근거"에 실측 수치를 붙일 수 있다
- 다음: 인스턴스 그래프 생성 범위 결정 후 `pipeline/build_knowledge_graph.py` 작성, NCP 계정·크레딧 신청, CLOVA Studio API 키 발급
