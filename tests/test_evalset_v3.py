# -*- coding: utf-8 -*-
"""블라인드 평가셋 v3 무결성 — 80문항·검사표 구조·검사 SQL 실행 가능(8/26).

v1(105/105)·v2(80/80) 포화 후의 새 시험지. 생성기(make_evalset_v3.py)는 seed 고정이라
재생성해도 같은 파일이 나와야 하고, 검사표 SQL 은 전부 실행돼야 한다.
"""
import io
import json
import os
...
EVAL = os.path.join(ROOT, "evalset", "evalset_v3.jsonl")
CHECKS = os.path.join(ROOT, "evalset", "checks_v3.jsonl")
...
def test_v3_structure():
    ...
    assert sum(1 for it in items if it["id"].startswith("V3-P-")) == 25        # 표현 변형 25
    assert sum(1 for it in items if it["id"].startswith("V3-C-")) == 14        # 3단 순회 14
    for it in items:
        assert it["question"].strip() and it["id"].startswith("V3-")
        ...
        for ch in row["checks"]:
            for c in _flatten(ch):
                ...
                if c["type"] in ("sql_names", "sql_number"):
                    con.execute(c["sql"]).fetchall()      # 실행 가능해야 한다(any_of 갈래는 비어도 됨)
                    any_rows = True
        assert row["checks"], row["id"]
